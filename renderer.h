...
template <typename T_in, typename T_out>
T_out inverse_scale(T_in x, T_in x_min, T_in x_max, T_out a, T_out b) {
...
namespace palette {
namespace helper {
sf::Color lerp_color(sf::Color a, sf::Color b, double factor) {
    return {
        lerp(a.r, b.r, factor),
        lerp(a.g, b.g, factor),
        lerp(a.b, b.b, factor)
    ...
    sf::Color simple(const int i, const int N) {
        if(i != N) {
            return {0,0,0};
        } else {
            return {255,255,255};
    ...
    sf::Color grayscale(const int i, const int N) {
        if( i != N ){
    ...
    sf::Color ultra_fractal(const int i, const int N) {
